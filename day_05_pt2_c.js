...
for (mapName in maps) {
  ...
  for (mapRangeNumber of mapRangeNumbers) {
    let [newOrigin, mapRange] = [mapRangeNumber[0], returnRange(mapRangeNumber[1], mapRangeNumber[2])]  
...
function applyMapToRanges(rangeA, rangeB, newOrigin){
  ...
  //A & B intersect, with A on the left and B on the right [ A [ ] B ]
    ...
    let [left, center, right] = [[startA, startB-1], [startB, endA], [endA+1, endB]]
  ...
  //B & A intersect, with B of the left and A on the right [ B [ ] A ]
    ...
    let [left, center, right] = [[startB, startA-1], [startA, endB], [endB+1, endA]]
  ...
  //A is a superset of B [ A [ B ]]
    ...
    let [left, center, right] = [[startA, startB-1], [startB, endB], [endB+1, endA]]
