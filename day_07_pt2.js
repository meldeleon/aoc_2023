...
function evaluateHand(str) {
  let hand = str.split("")
  let numberOfJokers = hand.filter((x) => x === "J").length
  let matches = []
  ...
  for (let i = 0; i < hand.length; i++) {
    ...
    for (let j = 0; j < cardTypes.length - 1; j++) {
      if (currentCard === cardTypes[j]) {
        matches[j]++
      }
    }
  }
  let sortedMatches = matches.sort((a, b) => b - a)
  if (numberOfJokers > 0) {
    //console.log(`${numberOfJokers} jokers found in hand ${str} `)
    sortedMatches[0] += numberOfJokers
  }
